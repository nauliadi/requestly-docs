...
### Parts of Replace Rule
<img src="/img/docs/annotated-replace-rule.png" alt="Replace Rule"/>

1. **Replace From** : This is the text the request which is replaced.
2. **Replace With** : This is what `Replace from` text is replaced with.
3. **Source Url** : This is the *source condition* when rule is applied. You can use `URL`, `Host` or `Path` with `Regex`, `Contains`, `Wildcard` or `Equals` to match the source request.
