The Map Remote feature lets you change the request's location to a new destination location such that the response 
is transparently served from the new location as if it was the original request.
...
All the Requestly rules can match requests based on the URL, host, path, and [rule operators](/browser-extension/chrome/features/rule-operators). With the help of these operators, you
can define rules to match using the wildcard, regex, or string matching types. You can also perform [advanced targeting](/browser-extension/chrome/features/advance-targetting) on the requests.
...
![Switch domain to a staging environment using replace rule](/img/docs/switch-domain-to-staging.png)
